...
function Adminjackets() {
    ...
    const fetchjackets = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/jackets/get-all-jackets"
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/jackets/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/jackets/${editProduct._id}`,
        formData
      );
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/jackets/add-jackets",
        formData,
        {
          headers: {
    ...
    <div className="min-h-screen py-6">
      <div className="container px-6 mx-auto">
        <h1 className="mb-6 text-3xl font-bold text-center text-gray-800">
          Admin Dashboard - Manage jackets
        </h1>
