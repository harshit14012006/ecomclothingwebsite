...
function AdminShirts() {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/shirts/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/shirts/${editProduct._id}`,
        formData
      );
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/shirts/add-shirts",
        formData,
        {
          headers: {
